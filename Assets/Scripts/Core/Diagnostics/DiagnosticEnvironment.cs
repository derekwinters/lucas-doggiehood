namespace Doggiehood.Core.Diagnostics
{
    ...
    /// this in from <c>Application</c>/<c>SystemInfo</c>/<c>Screen</c>; Core only
    /// formats it, which is exactly what keeps the whole payload deterministic
    /// under test.
    ///
    ...
    public readonly struct DiagnosticEnvironment
    {
        ...
        /// <summary>Which flavor is running, so a report from a store build is
        /// distinguishable from one off a development build.</summary>
        public string BuildFlavor { get; }

        /// <summary>Runtime platform name (e.g. "Android", "LinuxEditor").</summary>
