...
| Action | One positive/leaf pill button — **"Say hi!"** — which dismisses **and** pans the camera to the new house | [PillButton](shared-components.md#pill-button-pillbutton) |
...
- **The camera pan is the one non-presentational behavior.** Everything else on this panel is pure presentation, but **"Say hi!" pans the camera to the new house** via `CameraController.FocusOn` — so tapping it is truthful: the player is taken to meet their new neighbour. This reuses the existing `FocusOn` and is a slightly wider scope than [#373](https://github.com/derekwinters/lucas-doggiehood/issues/373) chose for zone-unlock (which deliberately only let the player *pan* to a new zone). **Flagged for the move-in implementation ([#436](https://github.com/derekwinters/lucas-doggiehood/issues/436)) to wire the pan alongside the panel dismiss.** Tapping the scrim dismisses without panning.
