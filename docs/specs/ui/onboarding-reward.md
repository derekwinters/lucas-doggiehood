...
Deliberately a **bespoke celebration panel**, not a reuse of the neutral [Confirmation dialog](confirmation-dialog.md) (Derek, 2026-07-29): it reads as a *reward* — a medal badge, a big "You did it!" heading, the coin payout as the hero element, a celebratory coin-burst — while still sitting on the shared Candy Cottage baseline and reusing the shared pill button.
...
| Medal badge | A round gold medal overlapping the panel's top edge, carrying an ink star — the "you earned it" marker | Reuses the Candy Cottage baseline (thick outline, hard shadow) from [Shared UI Components](shared-components.md) |
| Coin burst | A ring of small gold coins radiating from behind the medal — celebratory flourish (animated on device; static here) | — |
...
| Reward chip | The payout, called out large: a gold coin token + `+N` tabular amount | Coin token per [CurrencyChip](shared-components.md#currency-chip-currencychip) style |
| Action | One positive/leaf pill button — literal **"Yay!"** — that dismisses | [PillButton](shared-components.md#pill-button-pillbutton) |
...
| `MessageRewardGapPx` | `36` | Message → reward chip |
| `RewardCoinDiameterPx` | `72` | Gold coin token in the reward chip |
| `RewardAmountFontSizePx` | `56` | `+N` payout (tabular figures) |
| `RewardChipGapPx` | `16` | Coin token → amount |
| `RewardActionMarginPx` | `44` | Reward chip → action button |
| `ActionMinWidthPx` | `320` | The single **Yay!** pill (centered; grows with the label) |
| `BurstCoinDiameterPx` | `44` | Each coin in the celebratory burst |
| `BurstCoinCount` | `8` | Coins in the burst ring (decorative) |

The panel **chrome** (outline `OutlineThicknessPx` = 6 / corner radius `PanelRadiusPx` = 40 / hard drop-shadow) and the **Yay!** button (96 px [PillButton](shared-components.md#pill-button-pillbutton)) are owned by the [shared components](shared-components.md) ([#173](https://github.com/derekwinters/lucas-doggiehood/issues/173)) and not re-specified here. This page adds the celebration-specific composition — the medal, the coin burst, the heading, and the reward chip — and sizes them.

## Notes

- **One instance, reused for every onboarding step.** The caller passes the **message** (what was done) and the **amount** (`+100` today — `OnboardingRewardChainNumbers.RewardPerStep`); the heading and chrome are constant. This is the standard onboarding reward surface — no per-step panel ([#374](https://github.com/derekwinters/lucas-doggiehood/issues/374)).
...
- **Coin burst is motion, not layout.** The burst ring is a celebratory animation on device; the static mockup shows its resting placement. `BurstCoinCount`/`BurstCoinDiameterPx` size it, but no game logic depends on it.
