...
- **A house with bugs on it sprays instead of opening its profile ([#670](https://github.com/derekwinters/lucas-doggiehood/issues/670), Derek 2026-08-07).**

    > **How the spec is changing (#670).** It used to say a house tap always opens this profile → it now says a house tap opens this profile *unless* the house has an active pest-control quest, in which case the tap sprays it and the profile is unreachable until it's clear → because a house tap used to fire **both** handlers at once, spraying the house *and* opening this panel over the result.

    **Invariant — one house tap does exactly one thing.** Tapping a house either sprays it or opens its profile, never both, and the choice is made once from the house's own state (`HouseTapArbiter` in Core). While a [pest-control quest](../quests/quest-content.md) is active on a house, the whole house is the spray target and this profile is temporarily unreachable for it; spraying the last bug restores profile access on the very next tap. That keeps the pest-control rule — *"the house itself is the tap target… no separate spray tool or aiming"* — literally true, and is why the bug swarm needs no tap zone of its own.
