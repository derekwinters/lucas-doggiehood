...
- **Upgrade behavior ([#294](https://github.com/derekwinters/lucas-doggiehood/issues/294), Derek's Option A, 2026-07-26).** Tapping **Upgrade** spends coins **directly** through the Core entry point `GameState.TryUpgradeHouse(houseId)` — **no confirmation screen** (so #294 needed no new overlay/wireframe). Beyond the *Max level* disabled state, the button also **greys out and disables when the live wallet balance can't cover the next step**, reflecting affordability (`HouseProfile.CanAffordUpgrade`) read live against the wallet — the same "never cache" contract the currency HUD uses, never snapshotted. On a successful upgrade the open profile re-renders in place, so the new `Lv N` / filled pips / next cost / affordability show immediately. The affordable-vs-unaffordable disabled treatment reuses the same graybox disabled tint as *Max level* (no new chrome); the shared-component styling pass ([#173](https://github.com/derekwinters/lucas-doggiehood/issues/173)) owns its final look.
