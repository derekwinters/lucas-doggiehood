...
## Modal overlays block all input

> **How the spec is changing ([#670](https://github.com/derekwinters/lucas-doggiehood/issues/670)).** It used to say a modal blocks *taps* from reaching the world → it now says a modal blocks **every kind of input** from reaching the world *or the camera* → because "blocking" had only ever been built into the tap path: camera pan, pinch, twist and scroll never consulted the modal registry at all, so dragging a slider on the open debug tuning menu also panned the map underneath it.

**Invariant — while any modal is open, no input of any kind reaches the camera or the world.** Not taps, not drags, not pinches, not twists, not scroll. This is a rule about how input is wired, not a list of symptoms to fix: every gesture is resolved by the single [input authority](../../engineering/input-authority.md), which consults the modal registry once at press-down, so a new gesture kind cannot quietly arrive unblocked.

**Invariant — a gesture belongs to exactly one consumer, chosen at press-down, for its whole lifetime.** A drag that starts on a modal stays with that modal even after the pointer leaves it. A gesture already in flight when a modal opens keeps its owner until release; the modal blocks every gesture that *starts* after it opens.

Every dialog/menu overlay is **modal**: a tap that lands on it — on a button, on the panel's empty area, or on its dimmed backdrop — is consumed by the UI and **never** reaches a world interactable (a dog, a house, a lost item) behind it ([#422](https://github.com/derekwinters/lucas-doggiehood/issues/422)) — and, since [#670](https://github.com/derekwinters/lucas-doggiehood/issues/670), never reaches the camera either. Tapping **Accept** only accepts; tapping **Close** only closes; neither also fires whatever sat behind the button, and neither drags the neighborhood around under the panel.
...
The [input authority](../../engineering/input-authority.md) (see also [Camera & controls → Navigation](../world/camera-controls.md#navigation)) enforces the block two ways:

- **A shared modal registry (`ModalInputGate`, [#544](https://github.com/derekwinters/lucas-doggiehood/issues/544)).** Every modal surface — confirmation dialog, dog profile, house profile, welcome pop-up, onboarding reward, and the conversation panel ([#568](https://github.com/derekwinters/lucas-doggiehood/issues/568)) — registers itself as open on show and unregisters on close (and again if it is destroyed while open, so a torn-down overlay never leaves the block set). While any is registered (`IsBlocking`) the authority consumes the gesture at press-down, before any tier below it — world *or* camera — is offered it (#670; before that, only the tap-router asked). The registry also exposes a one-call reset used at hard boundaries — scene unload, or test isolation — so a stale registration can never leak past that boundary and leave world taps dead. This is a deterministic, frame-timing-independent flag: it is the authoritative guard, added because the original `EventSystem.IsPointerOverGameObject` signal alone could miss a fast **touch** tap-release over the scrim and leak that tap to the world object behind the panel (opening it and closing the profile).
- **A same-frame close latch (`ClosedThisFrame` / `EndFrame()`, [#568](https://github.com/derekwinters/lucas-doggiehood/issues/568)).** `IsBlocking` alone is a *live* snapshot of the open-token count, and a panel's `Unregister` runs **synchronously** while the `EventSystem` dispatches the very tap that dismissed it — but `EventSystem.Update()` and the camera rig's tap-routing `Update()` have no defined order. If the `EventSystem` runs first, `IsBlocking` already reads `false` for that same tap, which then falls through and fires whatever world object sat under the panel (the reported "one tap closes the window **and** opens the object underneath" bug). So `Unregister` of a still-open modal **latches** `ClosedThisFrame`, the tap-router blocks on `IsBlocking || ClosedThisFrame`, and `InputRouter.LateUpdate` calls `EndFrame()` to clear the latch — Unity runs every `LateUpdate` only after all `Update`s have completed, so the closing tap stays consumed for the rest of its frame regardless of ordering, while the next frame's genuinely-new tap routes normally. A no-op `Unregister` (unknown / already-removed / null token) does **not** latch, so a frame with no modal open never dead-ends a world tap.
...
Blocking suppresses only the *pass-through to the world and the camera*. A scrim/backdrop's own close/cancel button still fires through UGUI, so a panel that intentionally **dismisses on scrim tap** keeps doing so — "dismiss this panel" and "pass the tap through to the world" stay distinct.
