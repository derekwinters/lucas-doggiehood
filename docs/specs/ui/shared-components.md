...
- **A shared modal registry (`ModalInputGate`, [#544](https://github.com/derekwinters/lucas-doggiehood/issues/544)).** Every center-anchored overlay — confirmation dialog, dog profile, house profile, welcome pop-up, onboarding reward — registers itself as open on show and unregisters on close. While any is registered the tap-router swallows the tap before its world raycast. This is a deterministic, frame-timing-independent flag: it is the authoritative guard, added because the original `EventSystem.IsPointerOverGameObject` signal alone could miss a fast **touch** tap-release over the scrim and leak that tap to the world object behind the panel (opening it and closing the profile).
