...
| Action row | Two equal-width pill buttons: **No** (neutral/cream, left) and **Yes** (positive/leaf, right) | [PillButton](shared-components.md#pill-button-pillbutton) ×2 |
...
- **Buttons.** `No` is the neutral/cream decline on the **left** (the safe default a stray tap lands on); `Yes` is the positive/leaf confirm on the **right**. Per the shared [role-tint map](shared-components.md#shared-palette), the **confirm** button's tint is **overridable to coral** for a spend or destructive action (the zone-unlock spend may use coral); the labels default to **Yes/No** but a caller may override them (e.g. *"Unlock" / "Cancel"*). The decline stays neutral/cream in every case.
