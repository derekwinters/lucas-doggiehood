...
- **Portrait is a render-to-texture snapshot ([#464](https://github.com/derekwinters/lucas-doggiehood/issues/464), Derek's Option A, 2026-08-01).** The `PortraitSizePx` box shows a **live render of the dog's actual 3D model** — its shared breed model tinted to the dog's coat — not the flat placeholder color: an off-screen portrait camera (`PortraitCamera`) snapshots the model into the box **once on overlay-open**, not live every frame. This is a content-fill change within the already-approved box; the layout is unchanged.
