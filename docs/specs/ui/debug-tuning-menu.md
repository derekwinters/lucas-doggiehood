...
- **Dev-build-only visibility — the hard rule.** The **"Tune balance…"** entry row, and therefore this whole overlay, is compiled/shown **only in development builds**. It **must never appear in a release build** — not merely hidden behind the existing 10-tap Debug unlock, but absent from release entirely. The unlock gesture still gates the Debug *tab* itself as today; the dev-build guard is an additional, release-excluding gate on this row. As built ([#622](https://github.com/derekwinters/lucas-doggiehood/issues/622)) this is `DevBuildGate.IsDevBuild`: a `DEVELOPMENT_BUILD || UNITY_EDITOR` **build-symbol guard** that compiles to a constant `false` in a release player, deferring to `Debug.isDebugBuild` otherwise. Both `SettingsPanel.Init` and `TuningMenuOverlay.CreateIfDevBuild` take the answer as an injected `bool` — so a release build constructs neither the row nor a single overlay GameObject, and EditMode tests can exercise **both** sides of the gate (the Editor can only ever observe the dev side of the symbol itself).
