...
The geometry lives in Core (`YardLandscaping`), split the same two-tier way as `LotFence`: pure `GenerateFrontCandidates`/`GenerateBackCandidates`/`SelectFront`/`SelectBack` overloads take explicit yard regions and obstacles, while the `*For(HouseLot)` overloads resolve that geometry from `NeighborhoodLayout`/`LotFence` and derive the seed from the lot's own `HouseLot.HouseId`. Candidates are rejection-sampled inside [Property bounds](#property-bounds)' `LotBounds.FrontYard`/`BackYard`, collision-aware against the house footprint, the front walkway ([#128](https://github.com/derekwinters/lucas-doggiehood/issues/128)), the backyard fence line (checked via `LotFence.GeometryFor`, regardless of whether the fence is currently purchased/visible), and each other. Front candidates additionally stay clear of the road because the front-yard region itself now ends at the paved street corridor (`LotBounds.StreetCorridorInset`) rather than reaching the tile centerline — playtesting ([#244](https://github.com/derekwinters/lucas-doggiehood/issues/244)) had shown front trees escaping into the road, since a center that satisfied the generator's own footprint inset could still sit inside the road strip that the untrimmed region contained — every candidate and pick is spaced by `YardLandscaping.MinSpacing`, two footprint radii derived from the larger canopy half-extent of the two kit tree meshes (tree-large.fbx/tree-small.fbx, which share a canopy footprint) at the module's uniform scale, so no placed prop can ever overlap another regardless of which model lands where.
