...
`GameState.WalkNetwork` is the live network dogs actually wander ([#398](https://github.com/derekwinters/lucas-doggiehood/issues/398)): it is built (lazily, then cached) from the whole unlocked `Map` plus the lots of every **built, styled** house, and rebuilt whenever the map or houses change — after a successful `TryUnlockNextZone()` or `TryBuildHouse()` (and on save-restore). An expansion-built house with no authored art style ([#64](https://github.com/derekwinters/lucas-doggiehood/issues/64)) has no catalog door yet, so its lot is left off the graph (its tile's sidewalks/crosswalks still span it, since those derive from the map's roads, not its houses) — this also avoids the catalog throw a freshly unlocked, house-less zone lot would otherwise cause. The resulting network is fully connected across every unlocked tile — every node can reach every other node, including onto a newly unlocked cul-de-sac — which Core tests assert directly (BFS reachability). `NeighborhoodLayout.WalkNetwork` still exists as the bare starting-intersection network, used by Core tests that want it in isolation and as `WanderBehavior`'s parameterless-network default.
...
`WanderBehavior` ([Dog Behavior](../dogs/behavior.md)) no longer does centerline-bounce math on the streets directly. It now random-walks node to node over the sidewalk+crosswalk portion of the network — **front walkways are never entered**, so general wander never crosses onto a house lot, into a yard, or up to a door. The walk is over the **whole unlocked map** ([#398](https://github.com/derekwinters/lucas-doggiehood/issues/398)), so once a zone is unlocked dogs explore its sidewalks and crosswalks too, not just the starting intersection. A `WanderBehavior` (and `DogView`) binds to a **live network provider** (`() => state.WalkNetwork`) resolved on every hop, so an already-spawned dog picks up newly unlocked tiles the moment the map-derived network grows — no re-spawn.
