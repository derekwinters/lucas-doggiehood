...
**Invariant — a dog never reports "arrived home" anywhere but its own front door** ([#677](https://github.com/derekwinters/lucas-doggiehood/issues/677)). Sitting in the waiting pose *means* "I am at my door waiting for my delivery", so it is only ever entered at that door — the lot-side node of the dog's own house's front walkway. A dog is never sat down wherever its route happened to end, and a walk home that can't be planned is never resolved by pretending the dog arrived. The route is what carries the guarantee: it is planned over the **live** walk network and may end at that door or fail, never at a substitute destination (see [Sidewalks & Walk Network § Walking home](../world/sidewalks.md#walking-home-routes-over-the-network) for the routing rule and the "never leaves the walk network" invariant that governs the legs in between).

**A dog is never left waiting forever** ([#677](https://github.com/derekwinters/lucas-doggiehood/issues/677)). The waiting pose has exactly one exit — the delivery — so if that delivery cannot be carried out (no route home, no drivable road route to the door) it resolves *safely* instead of hanging: the item the player already paid for still arrives, and the dog is handed straight back to wander. See [Quest Content](../quests/quest-content.md).

> **How the spec is changing ([#677](https://github.com/derekwinters/lucas-doggiehood/issues/677)).** It used to say only that the dog "walks home at a medium speed, then sits and waits" — an outcome, with nothing said about *where* sitting is allowed → it now says the waiting pose belongs to the dog's own front door and nowhere else, and that a delivery which can't happen releases the dog rather than leaving it there → because a route planned on the wrong network ended on a sidewalk in the middle of the street, and the dog dutifully "arrived" and sat down there, waiting for a truck that could never be dispatched. Both halves of the old wording were satisfied by that bug.
...
- [x] The dog enters the waiting pose **only at its own front door** — a walk home planned over the live network ends there or fails loudly, every leg of it stays on the walk network (no beeline across yards to reach the route), and a delivery that can't be carried out releases the dog instead of leaving it sitting forever (#677)
