...
- **Pacing**: a rotation trickles the neighborhood's active requests in, mixing quest types, a fraction of the target each hour (`target / 4` per hour) rather than a fixed batch. Keeps sessions short and gives a reason to return. ([#26](https://github.com/derekwinters/lucas-doggiehood/issues/26), [#543](https://github.com/derekwinters/lucas-doggiehood/issues/543), [#624](https://github.com/derekwinters/lucas-doggiehood/issues/624))
    ...
    - **Per-hour trickle — a fractional accumulator**: the number added each hour is `target / PacingWindowHours` (a named `EconomyNumbers` constant, **4 hours** since [#624](https://github.com/derekwinters/lucas-doggiehood/issues/624)), so target 6 → 1.5/hr, 12 → 3/hr, and the floor of 5 → 1.25/hr. Fractional rates are handled by a **persisted error-diffusion accumulator** (`GameState.QuestPacingAccumulator`, round-tripped by `SaveCodec` — a legacy save with no `questPacingAcc=` line loads at 0.0): each hourly boundary it adds the rate, adds `floor(accumulator)` *whole* quests, and carries the remainder to the next hour. A quest is never fractional, quiet hours that add 0 are expected and fine, and the long-run rate equals `target / 4`. The per-refresh min/max fall out of this equation (min 0, max `ceil(perHour)`) rather than a fixed 2-4 batch.
...
- **Sources**: currency is earned only by completing dog requests/quests. No idle income, ads, or mini-games for v1.0 ([#24](https://github.com/derekwinters/lucas-doggiehood/issues/24)). There are two earn shapes:
