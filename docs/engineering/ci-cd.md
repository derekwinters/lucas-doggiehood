...
!!! tip "Testing in an emulator or Waydroid? Install the `-emulator.apk`"
    ...
    Before filing a graphics or ANR report against an emulator, confirm which one is actually installed: `adb shell pm list packages | grep doggiehood` should print `com.derekwinters.doggiehood.emulator`. Note that this check is itself under investigation — [#706](https://github.com/derekwinters/lucas-doggiehood/issues/706) tracks the `.emulator` suffix not appearing in ANR/process reports, so a bare `com.derekwinters.doggiehood` in a *logcat* is not on its own proof the wrong APK was used.
