...
Since CI builds APKs directly with `game-ci/unity-builder` (no exported Gradle project to add a `buildTypes { debug { ... } }` block to), the suffix is applied at Unity build time instead: an editor build hook (`Assets/Scripts/Unity/Editor/DebugApplicationIdBuildProcessor.cs`) appends `.debug` to `PlayerSettings.Android.applicationIdentifier` before the build and restores the permanent id afterward. Two channels can ask for it — the `DOGGIEHOOD_DEBUG_BUILD` environment variable for local and editor builds, and the `-doggiehoodDebugBuild` switch on Unity's command line for CI, which is the only one that reaches the editor inside game-ci's build container ([#734](https://github.com/derekwinters/lucas-doggiehood/issues/734); see [ci-cd.md](ci-cd.md#pr-debug-builds)). `pr-build.yml` and `rc-build.yml` pass the switch and then verify the finished APK's applicationId before uploading it; the release build asks through neither channel, so it always ships the bare `com.derekwinters.doggiehood` id.
...
> **How the spec is changing (#734).** This page used to say the debug-suffix hook applies `.debug` "whenever the `DOGGIEHOOD_DEBUG_BUILD` environment variable is set to a truthy value", and that `pr-build.yml` and `rc-build.yml` set it. It now says the hook takes two channels — that environment variable locally, and a `-doggiehoodDebugBuild` command-line switch in CI — and that both workflows verify the built APK before uploading it. The reason is that #734 found the environment variable never crossed into game-ci's build container, exactly as [#731](https://github.com/derekwinters/lucas-doggiehood/issues/731) found for the emulator flag, so no PR or RC artifact had ever actually carried the suffix.
