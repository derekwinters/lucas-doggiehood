...
namespace Doggiehood.Core.Tests.World
{
    ...
    /// <c>road-bend</c> connects its NORTH and WEST edges (==
    /// <see cref="TileType.TurnNW"/>); <c>road-crossroad-path</c> is the
    /// symmetric 4-way. The cul-de-sac's <c>road-end-round</c> open road exits
    /// +X in the raw kit source but WEST once Unity's FBX import mirrors the X
    /// axis, so <see cref="TileType.CulDeSacEast"/> takes a half-turn to bring
    /// the open road back to its declared EAST edge (#514). These are pure
