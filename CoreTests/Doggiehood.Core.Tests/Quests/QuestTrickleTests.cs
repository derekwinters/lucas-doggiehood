...
namespace Doggiehood.Core.Tests.Quests
{
    ...
    public class QuestTrickleTests
    {
        ...
        {
            // 8 dogs -> target 3 -> 0.5/hr. The first boundary adds floor(0.5)=0
            // whole quests and carries 0.5 on the accumulator; the second adds 1
            // and carries 0.0 — the remainder lives on GameState between calls.
            var state = StateWithDogs(8);

            state.Quests.MaybeStartNewDay(T0, new Random(1));
            Assert.That(state.Quests.ActiveQuests.Count(), Is.EqualTo(0), "0.5/hr adds nothing the first hour");
            Assert.That(state.QuestPacingAccumulator, Is.EqualTo(0.5).Within(1e-9), "the 0.5 remainder is banked");

            state.Quests.MaybeStartNewDay(T0 + EconomyNumbers.RefreshInterval, new Random(2));
            Assert.That(state.Quests.ActiveQuests.Count(), Is.EqualTo(1), "the carried 0.5 + 0.5 makes a whole quest");
            Assert.That(state.QuestPacingAccumulator, Is.EqualTo(0.0).Within(1e-9), "and the remainder returns to 0");
        }
        ...
        {
            var state = StateWithDogs(8);
            state.Quests.MaybeStartNewDay(T0, new Random(1)); // banks 0.5
            Assert.That(state.QuestPacingAccumulator, Is.EqualTo(0.5).Within(1e-9), "precondition: a pending fraction");

            var loaded = SaveCodec.Load(SaveCodec.Save(state));

            Assert.That(loaded.QuestPacingAccumulator, Is.EqualTo(0.5).Within(1e-9),
