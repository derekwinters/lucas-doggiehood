...
- **Resolved commands** (rendered per-repo from the distribution registry):
  - Test/red-green loop command: `dotnet test CoreTests/Doggiehood.Core.Tests`
  - Validate/verify/build command: `dotnet test CoreTests/Doggiehood.Core.Tests`
...
- Run with `dotnet test CoreTests/Doggiehood.Core.Tests` (or the repo's resolved test command).
...
### 5. Validate
- Run the repo's validator/build: `dotnet test CoreTests/Doggiehood.Core.Tests` (or the
...
- `dotnet test CoreTests/Doggiehood.Core.Tests` (or resolved validator) and the full suite pass ✓
