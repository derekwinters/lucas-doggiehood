...
class TestExclusionsAndHealth(unittest.TestCase):
    ...
    def test_empty_and_healthy_board_no_findings(self):
        ...
        for key in ("strip_labels", "requeue", "requeue_triage", "flag_done",
                    ...
                    "flag_orphaned_analysis", "flag_stuck_triage"):
...
# The real hand-back comments from #683 and #684 (issue #710's regression
# fixtures), kept verbatim as files under `tests/fixtures/` rather than inlined
# — they are long, and a byte-faithful copy is the whole point. Each writes its
# marker as a HEADING with no colon (`## ❓ Needs from Derek/Lucas`), which the
# colon-requiring recognizer missed; and since the needs-clarification route
# never emits a `## Build checklist`, that heading is the comment's ONLY
# signature, so the miss re-fired triage every cron sweep (#683 once, #684 three
# times).
def _fixture(name):
...
ISSUE_683_HEADING_MARKER_ANALYSIS = _fixture(
    "issue_683_comment_5224846989.md")
ISSUE_684_HEADING_MARKER_ANALYSIS = _fixture(
    "issue_684_comment_5224848201.md")
# #684's third pass: a short "still waiting on your ruling" note carrying NO
# marker in any shape. Nothing a signature recognizer can widen to reaches it —
# it is the case the `flag_stuck_triage` loop guard exists for.
ISSUE_684_MARKERLESS_RETRIAGE = _fixture(
    "issue_684_comment_5240580713.md")
...
class TestAnalysisSignature(unittest.TestCase):
    ...
    # --- heading form of the marker, no colon (issue #710) -----------------
    def test_heading_form_marker_matches(self):
        # The shape triage writes on the needs-clarification route: the marker
        # promoted to a heading, which drops the trailing colon the recognizer
        # required. `## Build checklist` is never emitted on that route, so this
        # heading is the comment's only signature.
        self.assertTrue(reconcile.has_analysis_signature(
            "## ❓ Needs from Derek/Lucas\n\n**Primary question:** which gate?"))

    def test_heading_form_marker_any_level(self):
    ...
    def test_issue_683_heading_marker_comment_matches(self):
        # Regression fixture: the real #683 comment that flapped once.
        self.assertNotIn("Build checklist", ISSUE_683_HEADING_MARKER_ANALYSIS)
        self.assertIn("## ❓ Needs from Derek/Lucas",
                      ISSUE_683_HEADING_MARKER_ANALYSIS)
        self.assertTrue(reconcile.has_analysis_signature(
            ISSUE_683_HEADING_MARKER_ANALYSIS))

    def test_issue_684_heading_marker_comment_matches(self):
        # Regression fixture: the real #684 comment that flapped three times.
        self.assertNotIn("Build checklist", ISSUE_684_HEADING_MARKER_ANALYSIS)
        self.assertTrue(reconcile.has_analysis_signature(
            ISSUE_684_HEADING_MARKER_ANALYSIS))

    def test_heading_form_still_requires_the_emoji(self):
        # Widening to the heading form must not drop the `❓` anchor either: a
        # heading that merely names the phrase is not a hand-back signature.
        for text in ["## Needs from Derek/Lucas",
                     "## **Needs from Derek/Lucas**",
                     "## Needs from Derek/Lucas: answered offline"]:
            self.assertFalse(reconcile.has_analysis_signature(text), text)

    def test_markerless_retriage_note_is_not_a_signature(self):
        # #684's short "still waiting on your ruling" note carries no marker at
        # all, so no recognizer widening can see it. Locked in deliberately:
        # this is the case `flag_stuck_triage` bounds instead of healing.
        self.assertFalse(reconcile.has_analysis_signature(
            ISSUE_684_MARKERLESS_RETRIAGE))
...
class TestStuckTriageGuard(unittest.TestCase):
    """`requeue_triage` is bounded, not unbounded (issue #710).

    `requeue_triage` is an unattended auto-fix, so a `has_analysis_signature`
    that cannot see an issue's analysis re-fires triage on it every cron sweep,
    forever — twice now (#100/#643 → #654, then #683/#684). Widening the
    recognizer fixes today's variant and re-arms the trap for the next one, so
    the rule now stops after `STUCK_TRIAGE_REQUEUE_LIMIT` unrecognized requeues
    with no owner comment in between and emits `flag_stuck_triage` instead: a
    recognizer gap degrades to a visible dashboard flag, not infinite churn.
    """

    def handback(self, count=None, **kw):
        kw.setdefault("labels", ["needs-clarification"])
        kw.setdefault("has_analysis_comment", False)
        if count is not None:
            kw["unrecognized_requeue_count"] = count
        return issue(684, **kw)

    def test_requeues_while_under_the_limit(self):
        for count in (0, 1, 2):
            out = run(payload([self.handback(count)]))
            self.assertEqual(numbers(out["requeue_triage"]), [684], count)
            self.assertEqual(out["flag_stuck_triage"], [], count)

    def test_flags_instead_of_requeueing_at_the_limit(self):
        out = run(payload([self.handback(3)]))
        self.assertEqual(out["requeue_triage"], [])
        self.assertEqual(numbers(out["flag_stuck_triage"]), [684])
        self.assertEqual(out["flag_stuck_triage"][0]["handback_state"],
                         "needs-clarification")

    def test_stays_flagged_beyond_the_limit(self):
        out = run(payload([self.handback(9, labels=["pending-approval"])]))
        self.assertEqual(out["requeue_triage"], [])
        self.assertEqual(out["flag_stuck_triage"][0]["handback_state"],
                         "pending-approval")

    def test_absent_count_behaves_as_zero(self):
        # `fetch_state` degrades to 0 on any timeline read error, and older
        # snapshots have no such key at all — either way the rule keeps healing.
        out = run(payload([self.handback()]))
        self.assertEqual(numbers(out["requeue_triage"]), [684])
        self.assertEqual(out["flag_stuck_triage"], [])

    def test_recognized_analysis_never_flags(self):
        # The guard only bounds the requeue path: an issue whose analysis IS
        # recognized is healthy, however many times it was requeued before.
        out = run(payload([self.handback(9, has_analysis_comment=True)]))
        self.assertEqual(out["requeue_triage"], [])
        self.assertEqual(out["flag_stuck_triage"], [])

    def test_events_only_emits_neither(self):
        # Same event-path caution as `requeue_triage` itself: a just-set
        # hand-back label can momentarily precede its comment's visibility, so
        # the event sweep neither requeues nor flags.
        out = reconcile.process(payload([self.handback(3)]), events_only=True)
        self.assertEqual(out["requeue_triage"], [])
        self.assertEqual(out["flag_stuck_triage"], [])

    def test_limit_is_three(self):
        # N = 3 is the flap count actually observed on #684 before #710 was
        # filed — a seen threshold, not a guess. Changing it is a spec change.
        self.assertEqual(reconcile.STUCK_TRIAGE_REQUEUE_LIMIT, 3)


class TestUnrecognizedRequeueCount(unittest.TestCase):
    """`unrecognized_requeue_count(timeline, owner_login)` (issue #710): the
    pure half of the `flag_stuck_triage` guard's input — how many automated
    `ai-triage` re-adds an issue has taken since the repo owner last commented.

    Counted from the issue timeline in chronological order: a bot-actor
    `ai-triage` label-add is one automated requeue; a comment by the repo owner
    is the human signal that resets the window (the situation may have changed,
    so triage deserves another go). Triage's own label writes are authored by
    the owner, not a bot, so only the sweep's re-adds count.
    """

    @staticmethod
    def add(actor, label="ai-triage"):
    ...
    def count(self, timeline):
        return reconcile.unrecognized_requeue_count(timeline, "derekwinters")

    def test_empty_timeline_is_zero(self):
        self.assertEqual(self.count([]), 0)

    def test_counts_bot_ai_triage_adds(self):
        self.assertEqual(self.count([
            self.add("github-actions[bot]"),
            self.add("github-actions[bot]"),
            self.add("github-actions[bot]"),
        ]), 3)

    def test_owner_comment_resets_the_window(self):
        # #684's real shape: several bot requeues, then a comment, then more.
        self.assertEqual(self.count([
            self.add("github-actions[bot]"),
            self.add("github-actions[bot]"),
            self.comment("derekwinters"),
            self.add("github-actions[bot]"),
        ]), 1)

    def test_non_owner_comment_does_not_reset(self):
        # The gatekeeper's own "Your move:" auto-comment is not a human signal.
        self.assertEqual(self.count([
            self.add("github-actions[bot]"),
            self.comment("github-actions[bot]"),
            self.add("github-actions[bot]"),
        ]), 2)

    def test_ignores_other_labels_and_events(self):
        self.assertEqual(self.count([
            self.add("github-actions[bot]", label="needs-clarification"),
            {"event": "unlabeled", "label": {"name": "ai-triage"},
             "actor": {"login": "github-actions[bot]"}},
            {"event": "referenced", "actor": {"login": "github-actions[bot]"}},
        ]), 0)

    def test_owner_label_add_is_not_an_automated_requeue(self):
        # Triage relabels under the owner's identity; a hand-back is not churn.
        self.assertEqual(self.count([self.add("derekwinters")]), 0)

    def test_malformed_entries_are_skipped(self):
        self.assertEqual(self.count([
            {}, {"event": "labeled"}, {"event": "labeled", "label": None},
            "not-a-dict", self.add("github-actions[bot]"),
        ]), 1)
